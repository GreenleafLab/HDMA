...
```{r}
# try to get exact P values from Wilcox
for (organ in c("Brain", "Heart", "Eye")){
  print("#####################")
  print(organ)
  df <- df_lognorm_z_sub
  # df <- df_lognorm_sub
  ...
  # 2. Calculate the tie correction term
  # Combine data to find ties across both groups
  all_data <- c(x, y)
  
  # t_j counts how many times each unique value appears
  t_j <- as.vector(table(all_data)) 
  
  # Calculate the summation part: sum((t_j^3 - t_j) / 12)
  tie_correction <- sum((t_j^3 - t_j) / 12)
  
  # 3. Calculate the adjusted Sigma_U using the formula
  # Term 1: (nx * ny) / (N * (N - 1))
  term1 <- (n1 * n2) / (N * (N - 1))
  
  # Term 2: (N^3 - N)/12 - correction
  term2 <- ((N^3 - N) / 12) - tie_correction
  
  sigma_U <- sqrt(term1 * term2)
  
  # 4. Calculate Z and P-value (One-tailed)
  Z <- (res$statistic - mu_U)
  Z_score <- Z / sigma_U
  
  # One-tailed log10 p-value
  log10_p <- pnorm(-abs(Z_score), log.p = TRUE) / log(10)
  print(log10_p)
}
```
