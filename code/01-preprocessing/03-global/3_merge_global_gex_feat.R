...
global.gexfeat.file <- paste0(wd, "/global_gexfeat_gr.rds")
...
if (!file.exists(global.gexfeat.file)){
  atac_proj <- loadArchRProject(paste0(wd, "/allSamples"))
  
  # take a look at the number of features in each arrow file
  n_gexfeat <- lapply(1:length(getArrowFiles(atac_proj)), function(i){
    ArchR:::.getFeatureDF(getArrowFiles(atac_proj)[i], "GeneExpressionMatrix")$name %>% length
  }) %>% unlist
  # > n_gexfeat
  # [1] 34735 34895 34895 34266 33518 34529 33518 34735 34010 34266 34314 34529 34895 33518 34341 34529 34529
  # [18] 34529 34341 33858 34341 33858 34341 33858 34529 34529 34314 33979 33518 33518 33518 34529 34529 34529
  # [35] 34314 33518 34529 34314 34314 34529 34314 33858 33858 34010 33858 33518 34010 34341 33979 33858 34529
  # [52] 33979 34529 33979 33979 34341 34529 34314 34010 34266 33979 34010 34010 33979 34010 30097 34314 34341
  # [69] 34735 30097 30097 28311 28311 28311 33979 28311
  
  global_gexfeat <- lapply(1:length(getArrowFiles(atac_proj)), function(i){
    ArchR:::.getFeatureDF(getArrowFiles(atac_proj)[i], "GeneExpressionMatrix")$name
  }) %>% unlist %>% unique
...
add_global_feat_wrapper <- function(organ, input.dir, global.gexfeat.file){
  ...
  atac_proj <- loadArchRProject(paste0(atac_wd, "/ATAC_obj_clustered_final/"))
  rna_proj <- paste0(rna_wd, "/RNA_obj_clustered_final.rds") %>% readRDS
  
  # save a copy of the atac proj to a new folder
  atac_proj_out <- paste0(atac_wd, "/ATAC_obj_clustered_final_common_gexfeat")
  atac_proj <- saveArchRProject(atac_proj, atac_proj_out, load=T)

  atac_proj <- addSeuratGeneExpressionMatrixFixedFeat(atac_proj, global.gexfeat.file, rna_proj)
...
replace_arrow_gex_mtx_wrapper <- function(input.dir, organ){
  atac_wd <- paste0(input.dir, "/", organ, "/atac_preprocess_output/")
  arrow_path <- paste0(atac_wd, "/ATAC_obj_clustered_final_common_gexfeat/ArrowFiles")
  arrow_files <- list.files(path = arrow_path, pattern = "\\.arrow$", full.names = TRUE)
  
  for (f in arrow_files){
    message(paste0("processing ", basename(f)))
    source_file <- f
    destination_file <- paste0(wd, "/allSamples/ArrowFiles/", basename(f))
