...
def main(args):
	...
	# PATHS ------------------------------------------------------------------------
	genome_fa_path = os.path.join(REFS_DIR, "hg38/chrombpnet/GRCh38_no_alt_analysis_set_GCA_000001405.15.fasta")
	...
	logger.critical(f"--- Processing Cluster: {cluster} ---")
	
	# Load models for this cluster once, and use throughout
	base_models = []
	if not args.downstream_only:
		logger.critical("\tLoading base BPNet models for all folds...")
		for fold_idx in range(5):
			model_path = os.path.join(PROJ_IN, f"01-models/models/bias_Heart_c0_thresh0.4/{cluster}/fold_{fold_idx}/models/chrombpnet_nobias.h5")
			model = BPNet.from_chrombpnet(filename=model_path)
			base_models.append(model)
	
		# Load background sequences ONCE per cluster.
		logger.critical("\tLoading background sequences...")
	...
	for row in compo_to_test.itertuples():
		...
		ch2 = logging.FileHandler(os.path.join(out_dir, f'{cluster}--{comp_motif_name_safe}--{datetimenow}.log'))
		...
		try:
			...
			if not args.overwrite and not args.downstream_only and not args.interpret and not args.interpret_only and os.path.exists(os.path.join(out_dir, ".done")):
				...
				logger.critical(f"{comp_motif_name} ({comp_type}) in {cluster} already done, skipping")
				...
				logger.critical(f"{cluster}: {comp_motif_name} ({comp_type})")
				...
				if not args.overwrite and os.path.exists(path_spacing_pkl):
					...
					# store summary dataframes/dfs
					spacing_dfs, spacing_preds = [], {}

					models = [ChromBPNetWrapper(m) for m in base_models]
				...
				if not args.overwrite and os.path.exists(path_solo_pkl):
					...
					with torch.no_grad():
						for key, motif in input_motifs_solo.items():
							...
							# pre-compute the edited sequences
							X_after_solo = substitute(X_nonpeaks, motif)

							y_before_list, y_after_list = [], []

							for model in models:
								y_before_fold = predict(model, X_nonpeaks)
								y_after_fold = predict(model, X_after_solo)

								# move and gather
								y_before_list.append((y_before_fold[0], y_before_fold[1]))
								y_after_list.append((y_after_fold[0], y_after_fold[1]))
				...
				if args.interpret or args.interpret_only:
					...
					if not args.overwrite_interpretation and os.path.exists(path_contribs_all_pkl) and os.path.exists(path_contribs_mean_pkl):
						...
						models_counts = [CountsWrapper(m) for m in base_models]
						...
						for model in models_counts:
							# run deep lift shap
							contributions_fold = deep_lift_shap(model, X_best.float(), random_state=0, batch_size=16)
							contributions_list.append(contributions_fold)
