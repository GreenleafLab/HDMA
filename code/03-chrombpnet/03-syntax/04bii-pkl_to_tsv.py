...
# Run on sherlock in the tangermeme env.
# e.g.
# $ orientations=("A,B HT" "B,A HT" "HH" "TT")
# $ for i in "${orientations[@]}"; do echo $i; python 04bii-pkl_to_tsv.py --mode counts --motif '74|BHLH_HD:PBX1/PDX1' --orientation "$i" --cluster Muscle_c0; done
# $ python 04bii-pkl_to_tsv.py --mode profile --motif '47|BHLH:TWIST/ZBTB_HD' --orientation 'A,B HT' --cluster Skin_c0
...
parser.add_argument("--cluster", type=str, default=None, help="Cluster name for ChromBPNet model")
...
out_path = os.path.join(PROJ_DIR, "output/03-chrombpnet/03-syntax/04b/in_silico_marginalization_full/", motif_safe, args.cluster)
print(out_path)
...
if args.mode == "profile":
	...
	# extract the data
	y_effects = spacing_preds[args.orientation]['y_after'][1] - spacing_preds[args.orientation]['y_before'][1]

	# subset to first 20 spacings
	y_effects = y_effects[:, :, 0:21, :]
