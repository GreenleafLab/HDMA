...
Firstly, for each composite motif, we focus on the cluster with the most instances, and evaluate
synergy in that cell type. We will assign motif pairs as having synergy or not, and having hard or soft
syntax (or both or neither).

Secondly, we run the *in silico* marginalizations for every composite motif in 
every cell type, to more generally assess specificity of the results and investigate
model predictions on motifs for TFs that are not expressed to be co-expressed.


# Set up

```{r libs, echo=TRUE, message=FALSE, warning=FALSE, cache=FALSE}
...
### Experiments in the cell type clusters of focus
...
orientations_hetero <- c("A,B HT", "B,A HT", "HH", "TT")
orientations_homo <- c("HH", "HT", "TT")
...
Similarly, for the set of tests of all motifs in all cell types, we compute difference
between joint and independent effects:
...
Similarly for the analysis of all motifs x all cell types, we simply assign each result
as synergistic or not:
...
```{r 47_spacing, fig.width = 7, fig.height = 6}

effects_47 <- map_dfr(orientations_hetero, ~ data.table::fread(
  file.path(ism_dir, glue("47|BHLH:TWIST.ZBTB_HD/Skin_c0/count_effects_per_fold_{.x}.tsv")),
  data.table = FALSE) %>% 
    mutate(orientation = .x))
...
spacing_df %>% 
  ...
  geom_point(data = effects_47, aes(y = effect), shape = 21) +
...
effects_74 <- map_dfr(orientations_hetero, ~ data.table::fread(
  file.path(ism_dir, glue("74|BHLH_HD:PBX1.PDX1/Muscle_c0/count_effects_per_fold_{.x}.tsv")),
  data.table = FALSE) %>% 
    mutate(orientation = .x))
...
spacing_df %>% 
  ...
  geom_bar(stat = "identity", color = "black", fill = "red") +
  # geom_errorbar(aes(ymin = effect_mean - effect_sd, ymax = effect_mean + effect_sd), width = 0.5) +
  geom_point(data = effects_74, aes(y = effect), shape = 21) +
  facet_wrap(~ orientation, ncol = 2)

```


# Marginalizations per motif across cell types
...
```{r dotplot_all_clusters, fig.width = 20, fig.height = 12}
...
Plot an annotation for motifs indicating the number of cell types 
where the motif is predicted to be synergistic:

```{r synergy_organ_barplot, fig.width = 15, fig.height = 5}
...
Plot an annotation for motifs indicating the distribution of effects
across cell types per motif:

```{r synergy_organ_boxplot, fig.width = 15, fig.height = 3}
