...
bubbleplot_data_decontx <- left_join(meanexp_scaled_decontx, pct1_decontx, by = c("cluster", "TF")) %>%
  # NOTE: here we filter to observations where a gene is detected in at least 1% of cells
  filter(Pct1 > 0) %>% 
...
For each motif and each cluster, the ISM has been run for 5 model folds and 100 
background sequences. The mean of effects across sequences is computed per fold,
and points are shown here.
...
plot_motif_across_celltypes <- function(motif, filter_statement, tf_rel_width = 1, top_n = 40) {
  ...
  agg_results_per_fold <- agg_results %>% 
    dplyr::select(L1_annot, matches("effect_fold")) %>% 
    pivot_longer(cols = matches("effect_fold"), names_to = "fold", values_to = "effect")
  ...
  p2 <- agg_results %>% 
    ...
    geom_point(data = agg_results_per_fold %>% filter(L1_annot %in% cluster_order), aes(y = effect), shape = 21) +
  ...
  #   dplyr::select(cluster, position, value) %>%
