...
out <- here::here("output/06-variants/07c/")
figout <- here::here("figures/06-variants/07c/")
dir.create(out, showWarnings = F, recursive=T)
dir.create(figout, showWarnings = F, recursive=T)
setwd(out)

rsid = "rs113892147"
gwas_id = "GCST90038616"
...
submeta = global_bp_obj$cell_metadata %>% 
  ...
  dplyr::filter(organ == "LU")
# Only has 1 cluster

# First show genes and GWAS SNPs
minStart = sub.hits$snpPos.hg38[1]-60000
maxEnd = sub.hits$snpPos.hg38[1]+35000

highlightStart = sub.hits$snpPos.hg38[1]-15000
highlightEnd = sub.hits$snpPos.hg38[1]+15000
...
p.gwas = ggplot() +
  ...
  geom_point(data = gwas.snps[gwas.snps$is.causal == "Y",], aes(x = start, y = score), color = "orangered1",
...
# 0-based coordinates
accpred = fread(paste0(here::here("output/06-variants/06/"),
                       "GCST90038616_D001249_Asthma_rs113892147_Lung_c12_peakcentered.tsv"),
                data.table = F, sep = "\t")
print(accpred$l2fc_mean) # -0.23
print(accpred$diff_local_mean) # -10.878
...
varscore_bed = rtracklayer::import.bed(paste0(here::here("output/06-variants/06/"),
                                              "GCST90038616_D001249_Asthma_rs113892147_Lung_c12_peakcentered.bed"), ## ----- Change this
                                       extraCols = c(
                                         "pred_ref" = "numeric",
                                         "pred_alt" = "numeric",
...
# Center at the variant, then expand 500 on both sides unless peak ends early
...
#### Fetal peaks ####
minStart = sub.hits$snpPos.hg38[1]-15000
maxEnd = sub.hits$snpPos.hg38[1]+15000
...
filelist = filelist %>% 
  filter(tissue == "lung" & peak_type == "pseudoreplicated peaks" & grepl("macrophage", celltype)) ## -------------------------- Change this
...
hits_input = unique(c(global_bp_obj$hits$Lung_c12))
...
# Each adult peak track gets 0.0125; there are 20
